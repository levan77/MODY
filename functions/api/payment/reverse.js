...
// Self-contained (no imports). Guarded: only when booking.status = 'cancelled'.
...
export async function onRequestPost({ request, env }) {
  try {
    ...
    if (bk.status !== 'cancelled') return Response.json({ error: 'Booking is not cancelled' }, { status: 409 });
    if (bk.payment_status !== 'authorized') return Response.json({ ok: true, note: 'No held funds to release' });
    ...
    if (r.response_status !== 'success') return Response.json({ error: 'Reverse failed', detail: json }, { status: 502 });

    const nowIso = new Date().toISOString();
    await sb.patch('payments', `id=eq.${pay.id}`, { status: 'voided', voided_at: nowIso, raw: r, updated_at: nowIso });
    await sb.patch('bookings', `id=eq.${encodeURIComponent(bookingId)}`, { payment_status: 'refunded' });
