...
function updateNav() {
  ...
  nb.innerHTML =
    ...
    "<span style=\"background:rgba(212,175,55,.15);color:var(--gd);padding:3px 10px;border-radius:50px;font-size:12px;font-weight:500\">" + rl + "</span>" +
...
// ═══════════════════════════════════════════════════════════════
//  PART 5: AUTH + CATEGORIES + PROFESSIONALS
// ═══════════════════════════════════════════════════════════════
...
// ═══════════════════════════════════════════════════════════════
//  PART 7: CLIENT DASHBOARD + PRO DASHBOARD
// ═══════════════════════════════════════════════════════════════
  ...
  try {
    ...
    if (proId) {
      r = await sb.from("professionals").update({
        name: name, specialty: spec, bio: bio, area: area, price_from: price, years_experience: years, status: "pending"
      }).eq("id", proId).select();
    } else {
      r = await sb.from("professionals").insert({
        user_id: user.id, name: name, specialty: spec, bio: bio, area: area, price_from: price, years_experience: years, status: "pending"
      }).select().single();
      if (r.data && r.data.id) {
        proId = r.data.id;
        await sb.from("profiles").update({ pro_id: proId }).eq("id", user.id);
...
// ═══════════════════════════════════════════════════════════════
//  PART 10C: DESIGN UPLOAD, ANALYTICS, LOOKUP
// ═══════════════════════════════════════════════════════════════
...
// ═══════════════════════════════════════════════════════════════
//  PART 10B: CALENDAR, TRACKER, ACCOUNT, UNIQUE IDS
// ═══════════════════════════════════════════════════════════════
