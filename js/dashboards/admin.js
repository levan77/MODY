...
async function exportAnalyticsExcel() {
  ...
  try {
    ...
    var sumRows = [
      ["AMODY Platform Analytics Report"],
      ...
      [""],
      ["STATUS BREAKDOWN",""],
      ["Pending", bks.filter(function(b) { return b.status === "pending"; }).length],
      ["Accepted", bks.filter(function(b) { return b.status === "accepted"; }).length],
      ["On the Way", bks.filter(function(b) { return b.status === "on_the_way"; }).length],
      ["Arrived", bks.filter(function(b) { return b.status === "arrived"; }).length],
      ["In Progress", bks.filter(function(b) { return b.status === "in_progress"; }).length],
      ["Completed", completed.length],
      ["Cancelled", cancelled.length],
      ["No Show", bks.filter(function(b) { return b.status === "no_show"; }).length],
      ["Late", bks.filter(function(b) { return b.status === "late"; }).length],
      ["Refunded", bks.filter(function(b) { return b.status === "refunded"; }).length]
    ];
    ...
    // ══ SHEET 7: MOST POPULAR SERVICES ══
    var allSvcCount = {};
    bks.forEach(function(b) {
      var sn = b.service_name || "Unknown";
      allSvcCount[sn] = (allSvcCount[sn] || 0) + 1;
    });
    var sortedSvcs = Object.keys(allSvcCount).map(function(k) { return { name: k, count: allSvcCount[k] }; })
      .sort(function(a, b) { return b.count - a.count; });
    var popRows = [["Rank","Service","Bookings","% of All Bookings"]];
    sortedSvcs.slice(0, 10).forEach(function(s, i) {
      popRows.push([i + 1, s.name, s.count, bks.length ? Math.round(s.count / bks.length * 100) : 0]);
    });

    // ══ SHEET 8: LEAST POPULAR SERVICES ══
    var leastRows = [["Rank","Service","Bookings","% of All Bookings"]];
    sortedSvcs.slice(-10).reverse().forEach(function(s, i) {
      leastRows.push([i + 1, s.name, s.count, bks.length ? Math.round(s.count / bks.length * 100) : 0]);
    });
    ...
    var xls = '<?xml version="1.0"?><?mso-application progid="Excel.Sheet"?>'
      ...
      + buildSheet("Most Popular Services", popRows)
      + buildSheet("Least Popular Services", leastRows)
      + '</Workbook>';
    ...
    a.href = url;
    a.download = "AMODY_Analytics_" + new Date().toISOString().slice(0,10) + ".xls";
    a.click();
    ...
    for (var i = 0; i < due.length; i++) {
      ...
      try {
        var msg = "Hi " + (item.client_name || "there") + "! It's been a while since your last "
          ...
          + ". Time to treat yourself again? Book at AMODY 💅";
